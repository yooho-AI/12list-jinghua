...
const P = 'jh'
const STORAGE_KEY = 'jinghua-chats'
...
你现在正在和对方闲聊。请完全以${char.name}的性格和说话方式回复。
规则：
- 用第一人称说话，不要用第三人称叙述
          ...
          {messages.map((msg, i) =>
            msg.role === 'user' ? (
              ...
              <div key={i} className={`${P}-npc-row`}>
                ...
                />
                <div className={`${P}-npc-bubble`}>{msg.content}</div>
              </div>
            ),
          )}
          ...
          {streaming && streamingContent && (
            <div className={`${P}-npc-row`}>
              <img
                ...
                style={{ borderColor: char.themeColor }}
              />
              <div className={`${P}-npc-bubble`}>{streamingContent}</div>
            </div>
          )}
